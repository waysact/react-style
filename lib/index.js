...
var StyleSheet = {
  ...
  matchMedia: function(query) {
    // TODO if no support ask to add polyfill
  ...
  assign: function(a, b) {
    var styleNames = Object.keys(b);
    for (var i = 0, l = styleNames.length; i < l; i++) {
      var styleName = styleNames[i];
      var style = b[styleName];
      var propNames = Object.keys(style);
      for (var j = 0, l2 = propNames.length; j < l2; j++) {
        var propName = propNames[j];
        a[styleName][propName] = style[propName];
      }
    }
  },
