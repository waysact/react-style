...
function createStyle(props, className, useClassName) {
  ...
  return !useClassName ||
         (typeof window !== 'undefined' && window.__ReactStyle__ && !__ReactStyle__[className]) ?
         style : className;
}

function createStyleNew(cssDef, useClassName) {
  var styles = Object.keys(cssDef);
  var results = {};
  for (var i = 0, l = styles.length; i < l; i++) {
    var styleName = styles[i];
    var style = cssDef[styleName];
    results[styleName] = createStyle(style, styleName, useClassName);
  }
  return results;
}
